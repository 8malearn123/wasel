...
export default function OnlineStorePage() {
  ...
  if (isLite) {
    const activePrimary = val("primary_color");
    return (
      <AppLayout title="تصميم المتجر" subtitle="باقة لايت — خصّص متجرك بخطوات بسيطة">
        {/* Status / publish bar */}
        ...
        <div className="max-w-3xl space-y-6">
          <div className="bg-card rounded-xl border p-6 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <Badge className="bg-primary/10 text-primary border-primary/30" variant="outline">باقة لايت</Badge>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5 mb-2">اختر أحد الألوان الجاهزة — التخصيص الكامل للألوان متاح في الباقات الأعلى</p>
              <div className="flex flex-wrap gap-3">
                {LITE_COLOR_PRESETS.map(p => (
                  <button key={p.name} type="button"
                    onClick={() => { set("primary_color", p.primary); set("secondary_color", p.secondary); }}
                    className={cn("flex items-center gap-2 rounded-xl border px-4 py-2.5 transition-all hover:border-primary/50",
                      activePrimary === p.primary ? "border-primary ring-2 ring-primary/30 bg-primary/5" : "border-border"
                    )}>
                    ...
                    {activePrimary === p.primary && <Check className="w-4 h-4 text-primary" />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <div className="rounded-xl border border-primary/30 bg-primary/5 p-5 flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-foreground mb-1">تبغى تحكم أكثر في متجرك؟</p>
              <p className="text-sm text-muted-foreground mb-3">
                باقة بلس وما فوق تفتح لك: أقسام المنتجات، الواجهة الرئيسية والبنرات، الصفحات التعريفية، إعدادات SEO، الشحن والدفع، وتخصيص كامل للألوان.
              </p>
              <Button size="sm" asChild>
                <a href="/subscription">ترقية الباقة</a>
              </Button>
            </div>
          </div>
        </div>
